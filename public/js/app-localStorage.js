...
function loadBooksFromStorage() {
    ...
    if (saved) {
        ...
        // Base de datos vacía - lista para agregar nuevos libros
        booksData = [];
        saveBooksToStorage();
    }
    return booksData;
            ...
            <div class="modal-body" style="padding: 2rem;">
                ...
                <form id="uploadForm" style="display: flex; flex-direction: column; gap: 1.5rem;">
                    ...
                    <div class="form-group">
                        ...
                        <input type="url" name="coverUrl" id="coverUrlInput" class="form-input" placeholder="https://drive.google.com/file/d/.../view" required>
                        <p class="form-hint">Pega el enlace de Google Drive de la imagen de portada</p>
                    </div>
                    <div class="form-group">
                        <label class="form-label">Fuente del Libro *</label>
                        <select name="sourceType" id="sourceType" class="form-select" onchange="togglePdfSource()">
                            <option value="drive">Enlace de Google Drive (Recomendado)</option>
                            <option value="file">Subir archivo PDF local</option>
                        </select>
                    </div>
                    <div class="form-group" id="driveUrlGroup">
                        <label class="form-label">Enlace de Google Drive *</label>
                        <input type="url" name="driveUrl" id="driveUrlInput" class="form-input" placeholder="https://drive.google.com/file/d/.../view">
                        <p class="form-hint">Pega el enlace de compartir del archivo PDF en Google Drive</p>
                    </div>
                    <div class="form-group" id="pdfFileGroup" style="display: none;">
                        <label class="form-label">Archivo PDF *</label>
                        <input type="file" name="pdf" id="pdfFileInput" accept="application/pdf" class="form-input">
...
// Función auxiliar para cambiar entre fuentes de PDF
function togglePdfSource() {
    const sourceType = document.getElementById('sourceType').value;
    const driveUrlGroup = document.getElementById('driveUrlGroup');
...
async function handleUploadSubmit(e) {
    ...
    if (!coverUrl || coverUrl.trim() === '') {
        alert('Debes proporcionar un enlace de Google Drive para la portada');
        return;
    }
    
    if (!window.DriveUtils || !window.DriveUtils.isDriveUrl(coverUrl)) {
        alert('El enlace de portada no es válido. Debe ser un enlace de Google Drive');
        return;
    }
    
    // Validar según el tipo de fuente del PDF
    if (sourceType === 'drive') {
        if (!driveUrl || driveUrl.trim() === '') {
            alert('Debes proporcionar un enlace de Google Drive para el libro');
            return;
        }
        
        // Validar que sea un enlace de Drive válido
        if (!window.DriveUtils.isDriveUrl(driveUrl)) {
            alert('El enlace del libro no es válido. Debe ser un enlace de Google Drive');
            return;
        }
    } else {
    ...
    try {
        // Procesar portada desde Google Drive
        const coverDriveInfo = window.DriveUtils.parseDriveUrl(coverUrl);
        ...
        let bookData = {
            ...
            coverUrl: coverDriveInfo.previewUrl, // URL de preview de Drive para la portada
            coverDriveFileId: coverDriveInfo.fileId,
            coverDriveUrl: coverDriveInfo.viewUrl,
            uploadedBy: AuthService.currentUser.uid,
            createdAt: new Date().toISOString(),
            hasPDF: true
        };
        
        // Manejar la fuente del PDF
        if (sourceType === 'drive') {
            // Usar Google Drive
            const driveInfo = window.DriveUtils.parseDriveUrl(driveUrl);
            bookData.driveFileId = driveInfo.fileId;
            bookData.driveUrl = driveInfo.viewUrl;
            bookData.drivePreviewUrl = driveInfo.previewUrl;
            bookData.driveDownloadUrl = driveInfo.downloadUrl;
            bookData.isFromDrive = true;
            bookData.pdfUrl = driveInfo.previewUrl;
            bookData.content = 'Google Drive: ' + driveInfo.fileId;
        } else {
            // Usar archivo local
            const pdfDataUrl = await readFileAsDataURL(pdfFile);
            bookData.pdfUrl = '';
            bookData.isFromDrive = false;
            bookData.content = 'PDF: ' + pdfFile.name;
